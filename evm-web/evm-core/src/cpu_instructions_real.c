 ...
 * Extracted from original Stcom.c
 * Contains 93 instruction handlers
 */
...
BYTE ccr = cpu.sregs.sr;
    ...
    // Inline assembly removed - not supported in WebAssembly
			...
			case 0:
				switch(of.special.size)
				{
					case 0:
						...
						{
							if(btemp&0x01)cpu.sregs.sr=cpu.sregs.sr|0x11;
							else cpu.sregs.sr=cpu.sregs.sr&(~0x11);
							btemp=(btemp>>1)|bmsb;
						}
					...
					case 1:
						...
						for(i=0;i<shiftcount;i++)
						{
							if(itemp&0x0001)cpu.sregs.sr=cpu.sregs.sr|0x11;
							else cpu.sregs.sr=cpu.sregs.sr&(~0x11);
							itemp=(itemp>>1)|imsb;
						}
						cpu.dregs.d[of.general.regsrc]=
					...
					case 2:
						...
						for(i=0;i<shiftcount;i++)
						{
							if(cpu.dregs.d[of.general.regsrc]&0x00000001L)
								cpu.sregs.sr=cpu.sregs.sr|0x11;
							else cpu.sregs.sr=cpu.sregs.sr&(~0x11);
							cpu.dregs.d[of.general.regsrc]=
			...
			case 1:
				switch(of.special.size)
				{
					case 0:
						...
						for(i=0;i<shiftcount;i++)
						{
							if(btemp&0x80)cpu.sregs.sr=cpu.sregs.sr|0x11;
							else cpu.sregs.sr=cpu.sregs.sr&(~0x11);
							btemp=((btemp<<1)&0xFE);
						}
						cpu.dregs.d[of.general.regsrc]=
					...
					case 1:
						...
						for(i=0;i<shiftcount;i++)
						{
							if(itemp&0x8000)cpu.sregs.sr=cpu.sregs.sr|0x11;
							else cpu.sregs.sr=cpu.sregs.sr&(~0x11);
...
void COM_bcc(short opcode)
{
	...
	{
		case 0:
			if( (cpu.sregs.sr&0x01) == 0)cpu.pc=cpu.pc+(short)GETword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+4;
			break;
		case 0xFF:
			if( (cpu.sregs.sr&0x01) == 0)cpu.pc=cpu.pc+(long)GETdword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+6;
			break;
		default:
			if( (cpu.sregs.sr&0x01) == 0)cpu.pc=cpu.pc+(char)(opcode&0x00ff)+2;
...
void COM_bhi(short opcode)
{
	...
	{
		case 0:
			if( (cpu.sregs.sr&0x05) == 0)cpu.pc=cpu.pc+(short)GETword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+4;
			break;
		case 0xFF:
			if( (cpu.sregs.sr&0x05) == 0)cpu.pc=cpu.pc+(long)GETdword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+6;
			break;
		default:
			if( (cpu.sregs.sr&0x05) == 0)cpu.pc=cpu.pc+(char)(opcode&0x00ff)+2;
			else cpu.pc=cpu.pc+2;
			break;
	}
}
...
void COM_ble(short opcode)
{
	...
	{
		case 0:
			if( (cpu.sregs.sr&0x04)||
				 ((cpu.sregs.sr&0x0a)==0x08)||
				 ((cpu.sregs.sr&0x0a)==0x02))cpu.pc=cpu.pc+(short)GETword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+4;
			break;
		case 0xFF:
			if( (cpu.sregs.sr&0x04)||
				 ((cpu.sregs.sr&0x0a)==0x08)||
				 ((cpu.sregs.sr&0x0a)==0x02))cpu.pc=cpu.pc+(long)GETdword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+6;
			break;
		default:
			if( (cpu.sregs.sr&0x04)||
			 ((cpu.sregs.sr&0x0a)==0x08) ||
			 ((cpu.sregs.sr&0x0a)==0x02))cpu.pc=cpu.pc+(char)(opcode&0x00ff)+2;
			else cpu.pc=cpu.pc+2;
			break;
	}
}
...
void COM_blt(short opcode)
{
	...
	{
		case 0:
			if( ((cpu.sregs.sr&0x0A)==0x08) || ((cpu.sregs.sr&0x0A)==0x02))
				cpu.pc=cpu.pc+(short)GETword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+4;
			break;
		case 0xFF:
...
void COM_bmi(short opcode)
{
	...
	{
		case 0:
			if( (cpu.sregs.sr&0x08) != 0)cpu.pc=cpu.pc+(short)GETword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+4;
			break;
		case 0xFF:
			if( (cpu.sregs.sr&0x08) != 0)cpu.pc=cpu.pc+(long)GETdword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+6;
			break;
		default:
			if( (cpu.sregs.sr&0x08) != 0)cpu.pc=cpu.pc+(char)(opcode&0x00ff)+2;
			else cpu.pc=cpu.pc+2;
			break;
	}
}
...
void COM_bvc(short opcode)
{
	...
	{
		case 0:
			if( (cpu.sregs.sr&0x02) == 0)cpu.pc=cpu.pc+(short)GETword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+4;
			break;
		case 0xFF:
			if( (cpu.sregs.sr&0x02) == 0)cpu.pc=cpu.pc+(long)GETdword(cpu.pc+2)+2;
			else cpu.pc=cpu.pc+6;
			break;
		default:
			if( (cpu.sregs.sr&0x02) == 0)cpu.pc=cpu.pc+(char)(opcode&0x00ff)+2;
			else cpu.pc=cpu.pc+2;
			break;
	}
}
...
{
short extension;
    // Removed ldiv_t - using long for division results instead
long Dr,Dq;
...
{
    // Removed ldiv_t - using long for division results instead
    long dv_quot, dv_rem;  // Division quotient and remainder
...
void COM_negx(short opcode)
{
	...
	work.destination=CommandMode[of.general.modesrc]((of.general.regsrc),0,0L,
																	of.special.size);
	cpu.pc=spc;
	work.destination=0-work.destination-(cpu.sregs.sr&0x10);
	CommandMode[of.general.modesrc]((of.general.regsrc),1,work.destination,
														of.special.size);
	switch(of.special.size)
	{
...
void COM_oritoCCR(short opcode)
{
	...
	cpu.pc+=2;
	cpu.sregs.sr=cpu.sregs.sr|(unsigned char)GETword(cpu.pc);
	cpu.pc+=2;
}
...
void COM_rtr(short opcode)
{
	...
	cpu.sregs.sr=(char)GETword(cpu.aregs.a[7]);
	cpu.aregs.a[7]+=2;
...
{
BYTE DecBorrow=0,ccr=cpu.sregs.sr;
BYTE Number1,Number2,Number;
...
{
char xtend=(cpu.sregs.sr&0x10)?1:0; // get extend flag from CCR
