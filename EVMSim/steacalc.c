...
long ARIPI(char reg,char command,long destination,char size)
{
	...
	switch(command)
	{
		case READ:
			...
			break;
		case WRITE:
			if(size==BYTE)PUTbyte(cpu.aregs.a[reg],(char)destination);
			else if(size==WORD)PUTword(cpu.aregs.a[reg],(short)destination);
			else PUTdword(cpu.aregs.a[reg],destination);
			cpu.aregs.a[reg]=cpu.aregs.a[reg]+(1<<size);
			break;
	}
	if(size==BYTE)return (GETbyte(cpu.aregs.a[reg]-(1<<size)));
	else if(size==WORD)return (GETword(cpu.aregs.a[reg]-(1<<size)));
	else if(size==DWORD)return (GETdword(cpu.aregs.a[reg]-(1<<size)));
	else return 0;
}
...
static long spc=0;
	...
	switch(command)
	{
		...
		case WRITE:
			...
			else if(size==DWORD)PUTdword(cpu.aregs.a[reg],destination);
			break;
	}
	switch(size)
	{
		...
		case 3: // PC relative with displacement and index 
			...
			switch(command)
			{
				case READ:
					...
					if(size==BYTE)return GETbyte((long)(t1+(long)(cpu.pc)-2+t2));
					else if(size==WORD)return GETword((long)(t1+(long)(cpu.pc)-2+t2));
					else if(size==DWORD)return GETdword((long)(t1+(long)(cpu.pc)-2+t2));
				case WRITE:
					break;
			}
			break;
